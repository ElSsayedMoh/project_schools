            ...
            <div class="card-body">
                ...
                @foreach(preg_split('/(-)/', $data[$counter]->answers) as $index=>$answer)
                    <div class="custom-control custom-radio">
                        ...
                        <label class="custom-control-label" for="customRadio{{$index}}" wire:click="nextQuestion({{$data[$counter]->id}}, {{$data[$counter]->score}}, '{{trim($answer)}}', '{{$data[$counter]->right_answer}}')">{{$answer}}</label>
                    </div>
                @endforeach
