...
@section('css')
    @toastr_css
@section('title')
{{trans('trans_school.Attendance_and_absence_list_for_students')}} 
@stop
@endsection
@section('page-header')
    <!-- breadcrumb -->
...
@section('content')
    ...
    @endif
    <h5 style="font-family: 'Cairo', sans-serif;color: red"> {{trans('trans_school.today_date')}}: {{ date('Y-m-d') }}</h5>
    ...
    <form method="post" action="{{ route('Attendance.store') }}">
        ...
        <table id="datatable" class="table  table-hover table-sm table-bordered p-0" data-page-length="50"
            ...
            <tr>
                ...
                <th class="alert-success">{{ trans('trans_school.Email') }}</th>
                <th class="alert-success">{{ trans('trans_school.gender') }}</th>
        ...
        <P>
            <button class="btn btn-success" type="submit">{{ trans('trans_school.Submit') }}</button>
        </P>
    </form><br>
    <!-- row closed -->
@endsection
